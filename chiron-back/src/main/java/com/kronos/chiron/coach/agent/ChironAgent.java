...
public interface ChironAgent {
    ...
    @SystemMessage({
            ...
            "",
            "MÉMOIRE LONG-TERME : un bloc « [MÉMOIRE LONG-TERME …] » peut précéder le message — considère ces notes comme acquises, utilise-les sans jamais les paraphraser. Sur info STRUCTURELLE révélée (blessure, douleur récurrente, préférence/contrainte forte, objectif chiffré, engagement) → [enregistrerNote] AUTOMATIQUEMENT, sans demander la permission, type adapté (BLESSURE, PREFERENCE, OBJECTIF, ENGAGEMENT, NOTE_LIBRE), contenu bref et factuel (le fait, pas tes conseils). Ne ré-enregistre pas une note déjà présente. Une note [SANTE] vient de Noctua (dérive de FC/VFC, dette de sommeil, charge cardio hors cible) — considère-la comme acquise, ne la réécris jamais toi-même. Oubli demandé → repère le #id dans la mémoire et [oublierNote] (sinon [getMesNotes] d'abord). Pour creuser au-delà des notes injectées → [getMesNotes] (filtre type optionnel).",
            "",
