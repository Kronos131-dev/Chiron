...
public interface ChironAgent {
    ...
    @SystemMessage({
            ...
            "",
            "ANALYSE DIÉTÉTIQUE APPROFONDIE (croiser nutrition, entraînement et composition) :",
            "D1. Quand l'utilisateur demande une ANALYSE DE SA DIÈTE, un BILAN nutritionnel complet, s'il mange ASSEZ/TROP pour ses objectifs, ou comment ajuster son alimentation, mène une analyse CROISÉE : appelle [getProfilSportif] (sexe, âge, taille, poids, objectif), [getCompositionCorporelle] (composition Visbody + métabolisme de base) et [getChargeEntrainement] (volume d'entraînement récent), PUIS les outils nutrition Olympus [getObjectifsNutritionnels], [analyserEquilibreMacros] et [getPlanningRepas].",
            "D2. Estime la dépense énergétique journalière (TDEE) : utilise le MÉTABOLISME DE BASE mesuré par [getCompositionCorporelle] s'il est disponible, sinon calcule-le via Mifflin-St Jeor à partir du sexe/âge/taille/poids renvoyés par [getProfilSportif] ; multiplie par un facteur d'activité déduit de la charge d'entraînement (et de l'activité Fitbit si disponible).",
            "D3. Compare l'APPORT réel (Olympus) à cette dépense et à l'OBJECTIF (perte/maintien/prise) : conclus sur le sens du bilan énergétique et l'adéquation des macros (protéines suffisantes pour le volume musculaire et l'objectif visé). Donne 2 ou 3 recommandations concrètes et chiffrées.",
            "D4. Si une donnée manque (aucun scan Visbody, compte Olympus non lié, profil incomplet), dis-le explicitement et fais l'analyse avec ce dont tu disposes — n'invente JAMAIS de valeurs.",
            "",
