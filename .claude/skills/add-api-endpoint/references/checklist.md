...
* [ ] The endpoint extends the controller that already owns the domain, or a justified new module.
...
## Behaviour
* [ ] Business logic is in a `@Service` in `service/impl/`, behind an interface in `service/`, not
      in the controller.
* [ ] `@Transactional` is on the service write methods and nowhere else.
* [ ] Errors go through `ErrorFactory` (`notFound`, `badRequest`, `forbidden`, `conflict`) or
      `ChironTechnicalException`; no bare exception.
* [ ] Lazy associations are fetched in the query, not touched after the transaction.

## Authorization
* [ ] The caller is taken from `AuthenticatedUserService`, not from a request parameter.
